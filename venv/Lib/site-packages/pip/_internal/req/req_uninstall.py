...
class StashedUninstallPathSet:
    ...
    def rollback(self) -> None:
        """Undoes the uninstall by moving stashed files src."""
...
class UninstallPathSet:
    ...
    def rollback(self) -> None:
        ...
        if not self._moved_paths.can_rollback:
            logger.error(
                "Can't roll src %s; was not uninstalled",
                self._dist.raw_name,
            )
            return
        logger.info("Rolling src uninstall of %s", self._dist.raw_name)
...
class UninstallPthEntries:
    ...
    def rollback(self) -> bool:
        if self._saved_lines is None:
            logger.error("Cannot roll src changes to %s, none were made", self.file)
            return False
        logger.debug("Rolling %s src to previous state", self.file)
        with open(self.file, "wb") as fh:
